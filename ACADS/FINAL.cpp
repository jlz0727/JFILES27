...
public:
    ...
    unordered_map<string, RoomNode> buildGraph(const vector<RoomInfo>& rooms) {
        ...
        for (const auto& a : rooms) {
            for (const auto& b : rooms) {
                ...
                // THIS LINE IS TO CONNECT ROOMS ON THE SAME FLOOR
                if (a.floorNum == b.floorNum) {
                    graph[a.fullRoom].neighbors.push_back({b.fullRoom, 1}); // THIS LINE IS TO SET A WEIGHT OF 1 FOR ROOMS ON THE SAME FLOOR
                }
    ...
    vector<string> dijkstraShortestPath(const unordered_map<string, RoomNode>& graph, const string& start) {
        ...
        // THIS LINE IS TO INITIALIZE OR TO FIND DISTANCES AND STARTING NODE
    ...
    void viewShortestPath() {
        ...
        for (const auto& request : requests) {
            if (request.getStatus() == "Pending") {
                ...
                int roomNum = 0;
                // THIS LINE IS TO EXTRACT ROOM NUMBER FROM FULL ROOM NAME
        ...
        // THIS LINE IS TO MATCH THE STARTING ROOM WITH PENDING ROOMS
...
private:
    ...
    map<string, vector<pair<string, vector<string>>>> buildings; // THIS DATA STRUCTURE LINE IS TO STORE BUILDINGS, FLOORS, AND ROOMS
    string lastStartRoom;

    // THIS LINE IS TO PRINT A REQUEST DETAILS
    ...
    // THIS LINE IS TO GET THE FULL ROOM NAME FROM LOCATION
    ...
    // THIS LINE IS TO EXTRACT BUILDING NAME FROM LOCATION
    ...
    // THIS LINE IS TO EXTRACT FLOOR LEVEL FROM LOCATION
